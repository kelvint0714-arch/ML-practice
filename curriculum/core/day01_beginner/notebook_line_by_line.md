# ESOL Notebook 逐行讲解（参考手册）

原 Notebook 有 10 个代码单元、212 行非空物理代码。为了让手机和 GitHub 页面更容易打开，逐行解释分为上下两部分；两部分合起来覆盖全部代码。

这份手册不再是 Day 1 必读任务。先完成算法概念、手算和最小代码；到 Day 7 运行完整 Notebook 时，只查询当时确实看不懂的部分。
...
Notebook 页面中的 `Branch` 和 `Source commit` 是上一次执行代码时写入的，例如：

```text
Branch: <运行时所在分支>
Source commit: <运行时的 Git 提交>
```

`.ipynb` 文件会把上次运行的输出文字一起保存；切换 Git 分支只更换文件状态，不会自动重新执行代码。因此，页面中的值可能与此刻终端所在分支不同。重新从空内核运行后，分支、提交和耗时会按本次运行更新；精确实验元数据以实验目录中的 `results/run_config.json` 为准。
