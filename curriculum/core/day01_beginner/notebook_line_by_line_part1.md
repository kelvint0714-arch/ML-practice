# ESOL 参考基线 E01 逐行讲解（第一部分）

> 本文是工程代码参考手册，不是当前课程 Day 1 的前置任务。先按 [一步一步学习目录](../../PROGRESS.md) 学算法；到课程 Day 7 查看完整基线时，只查询确实不理解的段落。文中的“Day 1”是旧实验编号。
...
重新运行后输出才会更新，但那也会形成一次新实验运行。不要把教程里的示意值抄进报告；精确源提交和工作区状态应读取实验目录中的 `results/run_config.json`。
