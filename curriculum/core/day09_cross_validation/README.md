...
`tutorial.ipynb` 是课程随附的确定性人工数据演示，不是学习者已经完成的实验。亲自运行、修改并解释后，再把本人的记录写入 `learning_outputs/day09_cross_validation/`。
...
真实下游任务数据到达后，需要根据分子骨架、配方号、批次或实验组考虑分组划分。不能为了得到更好分数临时选择分组字段。
