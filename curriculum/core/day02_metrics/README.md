...
- 已完成 [Day 1 算法概念与最小代码](../day01_beginner/README.md)；
...
今天仍然使用“概念 → 纸笔 → 最小代码”的顺序。先用四个数字完成误差表，确认自己会算以后，再运行代码；暂时不打开完整 ESOL Notebook。
...
R² 比较当前预测与“总是猜当前被评价这组 `y_true` 的平均值”之间的相对表现。这个数学参考均值不一定等于训练集均值；因此，用训练集均值产生预测的 Dummy 在外部验证集上也可能得到负 R²。
...
- 上一天：[Day 1 算法概念与最小代码](../day01_beginner/README.md)
- 完成验收后：[返回一步一步学习目录](../../PROGRESS.md)
- 下一天：[Day 3 Ridge](../day03_ridge/README.md)
