...
真实下游任务重复试样、同配方和同批次也需要分组，但 ESOL scaffold
不能冒充业务 group。真实分组字段必须由领域团队提供。

## 今日证据边界

若 Notebook 实际运行，只能描述固定 ESOL 协议的结果。不能把 OOF 的“无直接训练内预测泄漏”扩大成“没有任何可能泄漏”，更不能声称已完成真实下游任务 stacking。
