...
正式 v1.0 接口应怎样关联原料、配方、批次、试样、测试和表征，见 [粘合剂推荐数据结构](../DATA_SCHEMA.md)。v0.3 继续作为初步字段讨论稿，不在研究体系和目标确认前强行扩展成复杂数据库。
...
结构表示、描述符、数据分组和模型预测属于算法组生成字段，不应要求化学组手工填写。将来获得成对图像或谱图时，也必须通过批次或试样编号与性能记录关联。

数值缺失时保持单元格为空，并在说明或备注位置填写缺失原因；不能用 `0`、`-1` 或文字占据数值单元格。正式 v1.0 将使用独立缺失记录。
